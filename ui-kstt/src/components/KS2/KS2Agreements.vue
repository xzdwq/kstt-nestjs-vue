...
div
  ...
  fieldset(
    v-if="waitAgree.length > 0"
    class="flex p-2 border rounded-sm border-border-color-primary"
  )
    legend(class="flex items-center ml-2 px-2 text-sm font-bold") {{ $t('wait-agree') }}
      div(class="w-5 h-5 ml-2 flex items-center justify-center border border-border-color-primary rounded-md ")
        svg-down(
          ...
          :class="{'rotate-180': isOpenWaitAgree}"
          @click="isOpenWaitAgree = !isOpenWaitAgree"
        )
    div.flex.flex-wrap
      div(
        v-for="group of waitAndFutureAgree"
        :key="group.uuid"
        class="flex flex-wrap transition-all duration-700 overflow-hidden text-sm"
        :class="isOpenWaitAgree ? 'max-h-[300px]' : 'max-h-0'"
      )
        div(
  ...
  fieldset(
    ...
    legend(class="flex items-center ml-2 px-2 text-sm font-bold") {{  $t('agreed') }}
      div(class="w-5 h-5 ml-2 flex items-center justify-center border border-border-color-primary rounded-md ")
        svg-down(
          class="duration-300 cursor-pointer w-[17px]"
          :class="{'rotate-180': isOpenAgree}"
          @click="isOpenAgree = !isOpenAgree"
        )
    div.flex.flex-wrap
      div(
        v-for="type of agree"
        ...
        :class="isOpenAgree ? 'max-h-[300px]' : 'max-h-0'"
      )
        div(
...
export default {
  ...
  methods: {
    ...
    // Удалить еще не подписавшего согласованта
    onDelGroupConfirm(group) {
      this.modalCfg = {
        ...
        component: 'confirm',
        question: this.$t('del-approve', {name: group[`name_${this.$i18n.locale}`]}),
  ...
  async mounted() {
    ...
    // setTimeout(() => { console.log(this.waitMyAgree) }, 500)
  }
}
</script>