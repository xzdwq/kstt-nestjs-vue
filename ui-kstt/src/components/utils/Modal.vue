...
transition(name="fade")
  div(class="flex fixed z-10 inset-0 bg-black bg-opacity-40" v-if="modalCfg.modalShow" v-scroll-lock)
    div(class="flex justify-center items-center w-full" v-dragged="{ handle: '.handle' }")
        ...
        div(class="grid grid-rows-[max-content,1fr,max-content] min-h-full w-full")
          div(class="min-h-[40px]")
            div(class="flex flex-row justify-between")
              p(class="handle cursor-move w-full font-semibold select-none")
                slot(name="title")
              svg-close(class="cursor-pointer" @click="closeModal")
          div(class="flex-1 overflow-y-scroll")
            slot(name="body")
          div(class="h-[50px] pt-2 flex justify-between")
            div(class="handle cursor-move flex-1")
